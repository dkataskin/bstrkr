...
namespace bstrkr.mvvm.viewmodels
{
    public class VehicleForecastViewModel : BusTrackerViewModelBase, ICleanable
    {
        ...
        public RouteVehicleVMStates State => _stateMachine.State;
        ...
        private void Countdown()
        {
            this.Dispatcher.RequestMainThreadAction(() =>
            {
                lock (_lockObject)
                {
                    ...
                    {
                        Task.Factory.StartNew(() => _stateMachine.Fire(RouteVehicleVMTriggers.ForecastRequested), _cancellationToken);
                    }
                }
            });
        }
        ...
        {
            Task.Delay(TimeSpan.FromSeconds(30), _cancellationToken)
                .ContinueWith(task => _stateMachine.Fire(RouteVehicleVMTriggers.ForecastRequested), _cancellationToken);
        }
        ...
        {
            _messenger.Publish(new ShowVehicleForecastOnMapMessage(this, this.Vehicle.Id));
        }
    }
}