...
namespace bstrkr.core.android.services.location
{
                                   ...
                                   ILocationSource
    {
        ...
        private void InitializeGoogleAPI()
        {
            ...
            {
                var errorString =
                    $"There is a problem with Google Play Services on this device: {queryResult} - {apiAvailability.GetErrorString(queryResult)}";
        ...
        {
            this.LocationUpdated?.Invoke(this, new LocationUpdatedEventArgs(location.Latitude, location.Longitude));
        }

        private void RaiseLocationUpdatedEvent(double lat, double lon)
        {
            this.LocationUpdated?.Invoke(this, new LocationUpdatedEventArgs(lat, lon));
        }
    }
}